...
What could be causing this and how do I fix it?`,
  ...
  const analyzeComparison = (completedConfigs) => {
    ...
    const responseAnalysis = results.map(result => ({
      ...
      completenessScore: calculateCompletenessScore(result.response, selectedTemplate),
    }));
  ...
  const calculateReadabilityScore = (text) => {
    ...
    const avgWordsPerSentence = text.split(/\s+/).length / Math.max(text.split(/[.!?]+/).length, 1);
  ...
  const calculateCompletenessScore = (response, template) => {
    ...
    let bonusScore = 0;
    if (template?.tags?.includes('code') && response.includes('```')) bonusScore += 20;
    if (template?.tags?.includes('analysis') && response.includes('conclusion')) bonusScore += 15;
    if (response.includes('example') || response.includes('for instance')) bonusScore += 10;
  ...
  const generateInsights = (results, analysis) => {
    ...
    const speedDiff = Math.max(...results.map(r => r.duration)) - Math.min(...results.map(r => r.duration));
    if (speedDiff > 1000) {
      const fastest = results.find(r => r.duration === Math.min(...results.map(res => res.duration)));
      insights.push({
        type: 'performance',
        icon: BoltIcon,
        title: 'Speed Winner',
        description: `${fastest.model} is ${Math.round(speedDiff/1000)}x faster than the slowest model`,
        model: fastest.model,
    ...
    // Cost insights
    const costDiff = Math.max(...results.map(r => r.cost)) - Math.min(...results.map(r => r.cost));
    if (costDiff > 0.001) {
      const cheapest = results.find(r => r.cost === Math.min(...results.map(res => res.cost)));
      insights.push({
        type: 'cost',
        icon: CurrencyDollarIcon,
        title: 'Most Cost-Effective',
        description: `${cheapest.model} costs ${Math.round(costDiff * 10000) / 10000}$ less per request`,
    ...
    const qualityLeader = analysis.reduce((prev, curr) => 
      (prev.completenessScore + prev.readabilityScore) > (curr.completenessScore + curr.readabilityScore) ? prev : curr
    );
    insights.push({
      type: 'quality',
      icon: TrophyIcon,
  ...
  useEffect(() => {
    if (compareMode && testConfigs.length >= 2) {
      const completedConfigs = testConfigs.filter(config => config.result && !config.error);
                ...
                >
                  ...
                  {compareMode && (
                    <span className="bg-white/20 text-xs px-2 py-1 rounded-full">
                      {testConfigs.filter(c => c.result).length}/{testConfigs.length}
                    </span>
                  )}
                </button>
            ...
            {isFirstTime && (
              <div className="mb-12 text-center">
                ...
                <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-8">
                  Test any prompt across different AI models, compare responses, and find the perfect model for your use case. 
                  Start with a template or create your own custom test.
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-gray-600">Create a custom test with your own prompt and settings</p>
                </div>
              </button>
              ...
              <div className="group relative overflow-hidden bg-gradient-to-br from-emerald-50 to-blue-50 border-2 border-emerald-200 rounded-2xl p-8">
                ...
                <div className="relative">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">Use Templates</h3>
                  <p className="text-gray-600">Get started quickly with pre-built templates for common tasks</p>
                  <div className="mt-4 flex items-center space-x-2">
                    <StarIcon className="h-4 w-4 text-amber-500" />
                    <span className="text-sm text-emerald-700 font-medium">Recommended for beginners</span>
                  </div>
                </div>
              </div>
            </div>
                          ...
                          >
                            <div className="flex items-start justify-between mb-4">
                              <div className={`p-2 bg-white rounded-lg shadow-sm group-hover:shadow-md transition-shadow duration-200`}>
                                <TemplateIcon className={`h-5 w-5 ${colorClasses.icon}`} />
                              </div>
                            ...
                            <h4 className={`font-semibold mb-2 ${colorClasses.text}`}>{template.name}</h4>
            ...
            {savedPrompts.length > 0 && (
              <div className="mt-12 pt-8 border-t border-gray-200">
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Your Saved Prompts</h3>
                    <p className="text-sm text-gray-600">Quickly reuse your previous prompts</p>
                  </div>
                </div>
                  ...
                  {savedPrompts.slice(0, 6).map((saved) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-gray-900 truncate">{saved.name}</h4>
                        <DocumentTextIcon className="h-4 w-4 text-gray-400 group-hover:text-gray-600 transition-colors duration-200" />
                      </div>
                      <p className="text-sm text-gray-600 line-clamp-2 mb-3">{saved.prompt}</p>
                ...
                <div className="bg-gradient-to-r from-purple-600 via-blue-600 to-emerald-600 p-6 text-white">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold">Intelligent Comparison Analysis</h3>
                        <p className="text-white/90">AI-powered insights across {testConfigs.filter(c => c.result).length} model responses</p>
                      </div>
                    </div>
                    <button
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {comparisonMetrics.insights.map((insight, index) => {
                        ...
                        return (
                          <div key={index} className={`${insight.bg} border border-gray-200 rounded-xl p-4`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h5 className="font-semibold text-blue-900">Fastest Response</h5>
                          <BoltIcon className="h-5 w-5 text-blue-600" />
                        </div>
                        <div className="text-2xl font-bold text-blue-900 mb-1">
                          {comparisonMetrics.performance.fastest.duration}ms
                        </div>
                        <p className="text-sm text-blue-700">{comparisonMetrics.performance.fastest.model}</p>
                      </div>

                      <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h5 className="font-semibold text-green-900">Most Cost-Effective</h5>
                          <CurrencyDollarIcon className="h-5 w-5 text-green-600" />
                        </div>
                        <div className="text-2xl font-bold text-green-900 mb-1">
                          ${comparisonMetrics.performance.cheapest.cost.toFixed(4)}
                        </div>
                        <p className="text-sm text-green-700">{comparisonMetrics.performance.cheapest.model}</p>
                      </div>

                      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
                        <div className="flex items-center justify-between mb-3">
                          <h5 className="font-semibold text-purple-900">Highest Quality</h5>
                          <TrophyIcon className="h-5 w-5 text-purple-600" />
                        </div>
                        <div className="text-2xl font-bold text-purple-900 mb-1">
                          {Math.round(comparisonMetrics.quality.mostComplete.completenessScore)}%
                        </div>
                        <p className="text-sm text-purple-700">{comparisonMetrics.quality.mostComplete.model}</p>
                      </div>
                    </div>
                  </div>                  {/* Response Analysis */}
                  <div>
                    <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center space-x-2">
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-200">
                            ...
                            <th className="text-left py-3 px-4 font-semibold text-gray-900">Readability</th>
                          </tr>
                        </thead>
                        <tbody>
                              ...
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  <div className={`w-2 h-2 rounded-full ${
                                    item.duration === comparisonMetrics.performance.fastest.duration
                                      ? 'bg-green-500' : 'bg-gray-300'
                                  }`}></div>
                                  <span className="text-sm">{item.duration}ms</span>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  <div className={`w-2 h-2 rounded-full ${
                                    item.cost === comparisonMetrics.performance.cheapest.cost
                                      ? 'bg-green-500' : 'bg-gray-300'
                                  }`}></div>
                                  <span className="text-sm">${item.cost.toFixed(4)}</span>
                                </div>
                              </td>
                              <td className="py-3 px-4 text-sm">{item.tokens}</td>
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-sm">{item.completenessScore}%</span>
                                </div>
                              </td>
                              <td className="py-3 px-4">
                                <div className="flex items-center space-x-2">
                                  <div className="w-16 bg-gray-200 rounded-full h-2">
                                    <div 
                                      ...
                                      style={{ width: `${item.readabilityScore}%` }}
                                    ></div>
                                  </div>
                                  <span className="text-sm">{item.readabilityScore}%</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      {comparisonMetrics.detailed.slice(0, 2).map((item, index) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                          <div className="flex items-center justify-between mb-3">
                            <h5 className="font-medium text-gray-900">{item.model}</h5>
                            <div className="flex items-center space-x-2">
                              <span className="text-xs text-gray-500">{item.wordCount} words</span>
                              <span className="text-xs text-gray-500">•</span>
                              <span className="text-xs text-gray-500">{item.sentenceCount} sentences</span>
                            </div>
                          </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h5 className="font-medium text-gray-900 mb-2">Best for Production</h5>
                        <p className="text-sm text-gray-700">
                          Consider <strong>{comparisonMetrics.performance.fastest.model}</strong> for speed-critical applications
                          and <strong>{comparisonMetrics.performance.cheapest.model}</strong> for cost optimization.
                        </p>
                      </div>
                      <div>
                        <h5 className="font-medium text-gray-900 mb-2">Best for Quality</h5>
                        <p className="text-sm text-gray-700">
                          <strong>{comparisonMetrics.quality.mostComplete.model}</strong> provides the most comprehensive 
                          responses for complex tasks requiring detailed output.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {(config.result || config.error) && (
                    <div className="border-t border-gray-200/50 p-6">
                      {config.error ? (
                        ...
                        <div className="space-y-4">                          {/* Success Header */}
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                            ...
                            {compareMode && comparisonMetrics && (
                              <div className="flex items-center space-x-1">
                                {comparisonMetrics.performance.fastest.id === config.id && (
                                  <span className="inline-flex items-center px-2 py-1 bg-yellow-100 text-yellow-800 text-xs font-medium rounded-full">
                                    <BoltIcon className="h-3 w-3 mr-1" />
                                    Fastest
                                  </span>
                                )}
                                {comparisonMetrics.performance.cheapest.id === config.id && (
                                  <span className="inline-flex items-center px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                                    <CurrencyDollarIcon className="h-3 w-3 mr-1" />
                                    Cheapest
                                  </span>
                                )}
                                {comparisonMetrics.quality.mostComplete.id === config.id && (
